            ...
            :sortedRows.map(({inst,rate},idx)=>{
              ...
              return(
                <tr key={inst.name} style={{borderBottom:`1px solid ${s.light}`,background:isBest?"linear-gradient(135deg,#f0fdf4,#f7fdf9)":isVerified?"linear-gradient(135deg,#eff6ff,#f0f9ff)":idx%2===0?s.white:"#fafbfc"}}>
                  ...
                  <td style={{textAlign:"center",padding:"12px 10px"}}><a href={inst.url} target="_blank" rel="noopener noreferrer" style={{display:"inline-block",padding:"6px 12px",background:isBest?s.green:isVerified?"#1e40af":s.navy,color:"#fff",borderRadius:8,fontSize:11,fontWeight:700,textDecoration:"none"}}>{isVerified?"Apply at ACU →":"Apply →"}</a></td>
                </tr>
              );
            })}
          </tbody>
